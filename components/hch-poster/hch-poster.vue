	...
	export default{
		...
		methods:{
			...
			*/
			...
			         src: zpPriceIcon,
			         success(res) {
					//ctx.drawImage(res.path,65,380*scaleH,44, 15)//改宽高的
					ctx.drawImage(res.path,65,380*scaleH,15, 15)
					ctx.draw(true)
			      } ,
					fail(){
						_this.canvasFlag=true;
				...
				ctx.fillText('朗读：', 110, 396*scaleH,60);
				...
				ctx.fillText(zpPrice, 140, 396*scaleH,zpPriceW); //注意：这里调宽度
			
				ctx.beginPath();//开始一个新的路径
				ctx.setFontSize(16)//设置字体大小，默认10
				ctx.setFillStyle('#999')//文字颜色：默认黑色
				ctx.font = 'normal 12px sans-serif';
				ctx.fillText('积分：'+orignPrice, 150+zpPriceW+5, 395*scaleH,60);
				let orignPriceW = ctx.measureText(orignPrice).width//去掉市场价
				//ctx.fillText(orignPrice, 140+zpPriceW+15, 395*scaleH,orignPriceW); //5价格间距
				...
				ctx.font = 'normal bold 14px sans-serif';
				ctx.fillText('', (_this.phoneW-90)/2, 530*scaleH,90);
				// 小程序的名称end
				// 长按/扫描识别查看商品
				ctx.setFontSize(14)
				ctx.setFillStyle('#ff5f33')//文字颜色：默认黑色
				ctx.font = 'normal 14px sans-serif';
				ctx.fillText('长按/扫描识别小程序', (_this.phoneW-140)/2, 550*scaleH,140);
				// 长按/扫描识别查看商品end
				//绘制保存按钮
				ctx.save(); 
