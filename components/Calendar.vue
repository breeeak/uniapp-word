	...
	<view class='all'>
		<view class="bar">
			...
			<view class="previous" @click="handleCalendar(0)">
				<button class="cu-btn bg-gradual-blue" v-if="langType=='ch'">上一月</button>
				<button class="cu-btn bg-gradual-blue" v-else>Last</button>
			</view>
			<!-- 显示年月 -->
			<view class="date">{{cur_year || "--"}} 年 {{cur_month || "--"}} 月</view>
			<!-- 下一个月 -->
			<view class="next" @click="handleCalendar(1)">
				<button class="cu-btn  bg-gradual-blue" v-if="langType=='ch'">下一月</button>
				<button class="cu-btn  bg-gradual-blue" v-else>Next</button>
			</view>
		</view>
		<!-- 显示星期 -->
		...
		<view  class="myDateTable ">
			<view v-for="(item,j) in days" :key="j" class='dateCell'>
				<view v-if="item.date==undefined||item.date == null" class='cell text-cyan'>
					<text :decode="true">&nbsp;&nbsp;</text>
				</view>
				<view v-else>
					<!-- 已签到日期 -->
					<view v-if="item.isSign == true" class='cell whiteColor bgGray'>
						<text>{{item.date}}</text>
					</view>
					<!-- 漏签 -->
					<view @click="clickSignUp(item.date,0)" class="cell whiteColor  " v-else-if="cur_year<toYear||(cur_year==toYear&&cur_month<toMonth)||(cur_year==toYear&&cur_month==toMonth&&item.date<today)">
						<!-- 小程序不兼容这个 v-else-if="(new Date(cur_year+'-'+cur_month+'-'+item.date))<(new Date())"> -->
						<text>{{item.date}}</text>
					</view>
					<!-- 今日未签到-->
					<view @click="clickSignUp(item.date,1)" class="cell whiteColor" v-else-if="item.date==today&&cur_month==toMonth&&cur_year==toYear">
						<text>{{item.date}}</text>
					</view>
					<!-- 当前日期之后 -->
